...
<Tabs>
  <TabItem label="bun">
    ```sh
    bun add @spotify-effect/browser @spotify-effect/core effect
    ```
  </TabItem>
  <TabItem label="npm">
    ```sh
    npm install @spotify-effect/browser @spotify-effect/core effect
    ```
  </TabItem>
</Tabs>
...
Tokens are automatically refreshed when they expire. Call `refreshToken` to proactively refresh:
...
```ts
spotify.auth.logout()
// Clears tokens from session/localStorage
```
