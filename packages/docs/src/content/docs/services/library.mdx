...
```ts
const results = yield* library.areAlbumsSaved([
...
const exportLibraryToPlaylist = (userId: string, name: string) =>
  Effect.gen(function* () {
    ...
    const trackUris = savedTracks.map(item => item.track?.uri).filter(
...
const getAllSavedAlbums = () =>
  Effect.gen(function* () {
    ...
    return yield* paginateAll(
      (offset, limit) => library.getSavedAlbums({ offset, limit }),
      50,
    )
  })
```
